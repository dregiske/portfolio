...
export function renderDotField(
  ...
  // Globe: left of center, gently breathing, on a slow tilted spin.
  const cx = W * 0.34;
  const cy = H * 0.5;
  ...
  for (let n = 0; n < N; n++) {
    ...
    const persp = 0.32 + fz * 0.75;
    const fx = i / (gx - 1) - 0.5;
    const wave =
      (Math.sin(i * 0.45 + t) +
        Math.sin(j * 0.5 + t * 1.15) +
        Math.sin((i + j) * 0.3 + t * 0.7)) *
      (10 + fz * 26);
    const wx = W / 2 + fx * W * 1.25 * persp - mx * 34 * persp;
    const wy = H * 0.5 + fz * H * 0.78 + wave - my * 22;
    const wsize = 1.0 + fz * 2.1;
    const walpha = 0.12 + fz * 0.8;
    ...
    const rip = 1 + 0.05 * Math.sin(t * 0.9 + n * 0.15) + 0.03 * Math.sin(t * 1.7 + n * 0.05);
    ...
    // Arc the flight: bend perpendicular to the direction of travel, peaking
    // at the halfway point.
    const dxg = gsx - wx;
    const dyg = gsy - wy;
    const invg = 1 / Math.sqrt(dxg * dxg + dyg * dyg + 1);
    const curl = CURL[n] * 110 * Math.sin(pe * Math.PI);
    let sx =
      wx +
      dxg * pe -
      dyg * invg * curl +
      Math.sin(t * 1.5 + i * 0.16 + j * 0.11) * 14 * flux +
      Math.sin(t * 0.8 + i * 0.05 + j * 0.07) * drift;
    let sy =
      wy +
      dyg * pe +
      dxg * invg * curl +
      Math.cos(t * 1.3 + i * 0.12 + j * 0.17) * 14 * flux +
      Math.cos(t * 0.7 + i * 0.06 + j * 0.04) * drift;
    let size = wsize + (gsize - wsize) * pe;
    let alpha = walpha + (galpha - walpha) * pe;
    ...
    if (qe > 0.001) {
      ...
      const cyL =
        SY[k] * H +
        Math.sin(fxp * SFR[k] + t * STP[k] + SPH[k]) * SAMP[k] * H +
        Math.sin(fxp * SFR[k] * 0.35 + t * 0.18 + SPH[k]) * SAMP[k] * H * 0.5;
      const fyp = cyL + OFF[n] + Math.sin(t * 1.3 + n * 0.7) * 1.8 + my * 14;
      const dxc = fxp - sx;
      const dyc = fyp - sy;
      const invc = 1 / Math.sqrt(dxc * dxc + dyc * dyc + 1);
      // Head straight for the lane — just a whisper of curl so it stays organic.
      const curlq = CURL[n] * 36 * Math.sin(qe * Math.PI);
      const fluxq = qe * (1 - qe);
      sx = sx + dxc * qe - dyc * invc * curlq + Math.sin(t * 1.4 + i * 0.14 + j * 0.1) * 5 * fluxq;
      sy = sy + dyc * qe + dxc * invc * curlq + Math.cos(t * 1.2 + i * 0.1 + j * 0.15) * 5 * fluxq;
    ...
    if (re > 0.001) {
      ...
      const px2 =
        SX0[n] * W +
        mx * (7 + lay * 11) +
        Math.sin(t * 0.45 * TWS[n] + TWP[n]) * DRA[n] +
        Math.sin(t * 0.22 + TWP[n] * 1.7) * DRA[n] * 0.5;
      const py2 =
        SY0[n] * H +
        my * (5 + lay * 8) +
        Math.cos(t * 0.36 * TWS[n] + TWP[n] * 1.3) * DRA[n] * 0.8 +
        Math.cos(t * 0.18 + TWP[n] * 2.1) * DRA[n] * 0.4;
      const tw = 0.72 + 0.28 * Math.sin(t * TWS[n] + TWP[n]);
      const dxs = px2 - sx;
      const dys = py2 - sy;
      const invs = 1 / Math.sqrt(dxs * dxs + dys * dys + 1);
      const curlr = CURL[n] * 120 * Math.sin(re * Math.PI);
      const fluxr = re * (1 - re);
      sx = sx + dxs * re - dys * invs * curlr + Math.sin(t * 1.3 + i * 0.13 + j * 0.09) * 10 * fluxr;
      sy = sy + dys * re + dxs * invs * curlr + Math.cos(t * 1.1 + i * 0.09 + j * 0.14) * 10 * fluxr;
    ...
    se = se * se * (3 - 2 * se);
    if (se > 0.001) {
      const wy2 = H * 0.5 - fz * H * 0.78 + wave - my * 22;
      const dxw = wx - sx;
      const dyw = wy2 - sy;
      const invw = 1 / Math.sqrt(dxw * dxw + dyw * dyw + 1);
      const curls = CURL[n] * 110 * Math.sin(se * Math.PI);
      const fluxs = se * (1 - se);
      sx = sx + dxw * se - dyw * invw * curls + Math.sin(t * 1.5 + i * 0.16 + j * 0.11) * 14 * fluxs;
      sy = sy + dyw * se + dxw * invw * curls + Math.cos(t * 1.3 + i * 0.12 + j * 0.17) * 14 * fluxs;
